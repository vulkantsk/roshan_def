...
LinkLuaModifier( "modifier_shopkeeper", "core/libraries/modifiers/modifier_shopkeeper.lua", LUA_MODIFIER_MOTION_NONE )
...
ApplyPassives = function(container, item, entOverride)
  --print("[Containers] ApplyPassives(", container, item, entOverride, ")")
...
function unitInventory:AddItem(item, slot)
  print("[Containers]:AddItem(", item, slot, ")")
  item = GetItem(item)
...
function Containers:start()
  ...
  CustomGameEventManager:RegisterListener("Containers_OnAltLeftClick", Dynamic_Wrap(Containers, "Containers_OnAltLeftClick"))
  ...
  if item and unit then
    ...
    if defInventory then
      ...
      if not full or (full and item:IsStackable() and exists) then
        --print("[Containers]:AddItemToUnit - there was space")
        unit:AddItem(item)
      else
        --print("[Containers]:AddItemToUnit - there was NOT space")
        CreateItemOnPositionSync(unit:GetAbsOrigin() + RandomVector(10), item)
      end
    end
  end
...
function GetRarityColor(itemName)
  ...
  if itemDefinition == nil then
    itemDefinition = Containers.dotaItemKV[itemName]
  end

  local color = "white"
  if itemDefinition.ItemQuality == "common" then
    color = "green"
  elseif itemDefinition.ItemQuality == "rare" then
    color = "#0056B2"
  elseif itemDefinition.ItemQuality == "epic" then
    color = "purple"
  elseif itemDefinition.ItemQuality == "artifact" then
    color = "orange"
  end
...
function Containers:Containers_OnAltLeftClick(args)
  print('Containers_OnAltLeftClick')
  PrintTable(args)
  ...
  local itemID = args.itemID
  local slot =  args.slot
  local custInvCheck = args.custInv
  ...
  print("check d")
  local ent = container:GetEntity()
  local color = GetRarityColor(item:GetName())
  --Display ping notification

  print("container:IsInventory() == ", container:IsInventory())
  if ent then
    print("ent is " .. ent:GetUnitName())
  end
  print("ContID is " .. contID)

  if contID == 0 or ent == unit then
      if item:GetCooldownTimeRemaining() > 0 then
        print("check g")
        Notifications:BottomToAll({hero=unit:GetUnitName(), imagestyle="landscape", style={["width"]="80px", ["height"]="40px"}, duration=5.0})
        Notifications:BottomToAll({text="- 'My ", style={color="white", ["font-size"]="25px", ["margin-left"]="15px"}, continue = true})
        Notifications:BottomToAll({text=GetTrueName(item, "item") .. " ", style={color=color, ["font-size"]="25px"}, continue = true})
        Notifications:BottomToAll({text="is on cooldown for " .. math.floor(item:GetCooldownTimeRemaining() + 0.5) .. " seconds!'", style={color="white", ["font-size"]="25px", ["margin-right"]="150px"}, continue = true})
        Notifications:BottomToAll({item=item:GetName(), style={["wash-color"]="#3A2323", ["width"]="55px"}, continue=true})
        --Say( PlayerResource:GetPlayer(playerID), item:GetName() .. ": On cooldown " .. item:GetCooldownTimeRemaining() .. " seconds remaining!", false)
      else
        print("check h")
        Notifications:BottomToAll({hero=unit:GetUnitName(), imagestyle="landscape", style={["width"]="80px", ["height"]="40px"}, duration=5.0})
        Notifications:BottomToAll({text="- 'My ", style={color="white", ["font-size"]="25px", ["margin-left"]="15px"}, continue = true})
        Notifications:BottomToAll({text=GetTrueName(item, "item") .. " ", style={color=color, ["font-size"]="25px"}, continue = true})
        Notifications:BottomToAll({text=" is ready!'", style={color="white", ["font-size"]="25px", ["margin-right"]="150px"}, continue = true})
        Notifications:BottomToAll({item=item:GetName(), style={["width"]="55px"}, continue=true})
        --Say( PlayerResource:GetPlayer(playerID), "<font color='#C02BBE'>" .. item:GetName() .. "</font>" .. ": Ready!", false)
      end
    --end
  else
    --Notifications:TopToAll({image="file://{images}/status_icons/dota_generic.psd", duration=5.0})
    Notifications:BottomToAll({hero=unit:GetUnitName(), imagestyle="landscape", style={["width"]="80px", ["height"]="40px"}, duration=5.0})
    Notifications:BottomToAll({text="- '", style={color=color, ["font-size"]="25px", ["margin-left"]="15px"}, continue = true})
    Notifications:BottomToAll({text=GetTrueName(item, "item") .. " ", style={color=color, ["font-size"]="25px"}, continue = true})
    Notifications:BottomToAll({text=" here!'", style={color=color, ["font-size"]="25px", ["margin-right"]="150px",}, continue = true})
    Notifications:BottomToAll({item=item:GetName(), style={["width"]="55px"}, continue=true})

    --Say( PlayerResource:GetPlayer(playerID), item:GetName() .. " Here!", false)
  end
end
...
function Containers:Containers_OnLeftClick(args)
  ...
  local range = container:GetRange()
  local ent = container:GetEntity()
  --if range == nil and ent and unit ~= ent then return end   --Base container script, removed to add forceActivateItem
  if range == nil and ent and unit ~= ent and not container.forceActivateItem then return end
  if range and ent and unit and (ent:GetAbsOrigin() - unit:GetAbsOrigin()):Length2D() >= range then 
    print("g")
    Containers:DisplayError(playerID,"#dota_hud_error_target_out_of_range")
    return 
...
function Containers:CreateShop(cont)
  ...
  function shop:BuyItem(playerID, unit, item)
    --print("[Containers]:BuyItem(", playerID, unit, item, ")")
  ...
  if not cont.OnRightClick then
    shop:OnRightClick(function(playerID, container, unit, item, slot)
      ...
      print("[Containers] shop:OnRightClick(", playerID, container, unit, item, slot, ")")
...
function Containers:CreateContainer(cont)
  ...
  function c:ActivateItem(unit, item, playerID)
    if item:GetOwner() ~= unit or not item:IsFullyCastable() then 
      ...
      print(item:GetName(), "was not owned by", unit:GetUnitName())
      return
    end
    ...
    if noTarget and not channelled then
      ...
      CustomNetTables:SetTableValue("containers_lua", tostring(abil:GetEntityIndex()), {behavior=behavior, aoe=aoe, range=item:GetCastRange(unit:GetAbsOrigin(), unit), 
  ...
  function c:AddItem(item, slot, column, bypassFilter)
    --print("[Containers]: AddItem(", item, slot, column, bypassFilter, ")")
    item = GetItem(item)
    ...
    for i=slot,size do
      --print("[Containers]: AddItem: 2495", "- slot", i, "- size", size)
      local slotitem = self:GetItemInSlot(i)
      if not slotitem then
        --print("[Containers]: AddItem: 2498 - No slotitem")
        ...
        if self:IsEquipment() then
          --print("[Containers]: AddItem: 2516 - Applying Passives")

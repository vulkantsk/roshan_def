...
function BuildingHelper:SetupBuildingTable(abilityName, builderHandle)
    ...
    local bBuilderInside = GetUnitKV(unitName, "BuilderInside") or 0
    buildingTable:SetVal("BuilderInside", bBuilderInside)

    local bConsumesBuilder = GetUnitKV(unitName, "ConsumesBuilder") or 0
    buildingTable:SetVal("ConsumesBuilder", bConsumesBuilder)
...
--    local enemies = FindUnitsInRadius(unit:GetTeamNumber(), location, nil, construction_radius, DOTA_UNIT_TARGET_TEAM_ENEMY, target_type, flags, FIND_ANY_ORDER, false)
    local enemies = FindUnitsInRadius(unit:GetTeamNumber(), location, nil, construction_radius, DOTA_UNIT_TARGET_TEAM_BOTH, target_type, flags, FIND_ANY_ORDER, false)

    for _,enemy in pairs(enemies) do
        local origin = enemy:GetAbsOrigin()
		print(enemy:GetUnitName())
        if enemy:GetUnitName()~="npc_dota_greevil_worker" and not IsCustomBuilding(enemy) and BuildingHelper:EnemyIsInsideBuildingArea(enemy:GetAbsOrigin(), location, size)  then
            if callbacks.onConstructionFailed then
                callbacks.onConstructionFailed()
                return false
            end

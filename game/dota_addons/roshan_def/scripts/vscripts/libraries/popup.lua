...
function PopupRedDamage(target, amount)
    PopupNumbers(target, "xp", Vector(255, 0, 0), 3.0, amount, nil, POPUP_SYMBOL_POST_LIGHTNING)
end
...
function PopupGoldGain(target, amount,gain)
    gain = gain==nil and true or gain
    if gain then
        PopupNumbers(target, "gold", Vector(255, 200, 33), 2.0, amount, POPUP_SYMBOL_PRE_PLUS, nil)
    else
        PopupNumbers(target, "gold", Vector(255, 200, 33), 2.0, amount, POPUP_SYMBOL_PRE_MINUS, nil)
    end
end
function PopupGoldGainAlt(target, amount,gain)
    gain = gain==nil and true or gain
    if gain then
        PopupNumbers(target, "xp", Vector(255, 200, 33), 3.0, amount, POPUP_SYMBOL_PRE_PLUS, nil)
    else
        PopupNumbers(target, "xp", Vector(255, 200, 33), 3.0, amount, POPUP_SYMBOL_PRE_MINUS, nil)
    end
end

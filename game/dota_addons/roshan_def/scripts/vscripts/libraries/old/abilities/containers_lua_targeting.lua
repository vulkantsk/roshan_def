...
function containers_lua_targeting:CastFilterResultTarget( hTarget )
  ...
  local item = EntIndexToHScript(result.proxyItem)
  local castResult
  if item.CastFilterResultTarget then
    castResult = item:CastFilterResultTarget( hTarget )
  end
  if castResult then
    return castResult
  end
...
function containers_lua_targeting:OnSpellStart()
  ...
  local invId = item.myInventoryID
  local inv = InventoryService.allInventories[invId]
  ...
  if inv then
    local slotId = inv.items[itemId]

    CustomGameEventManager:Send_ServerToPlayer(PlayerResource:GetPlayer(owner:GetPlayerOwnerID()), "refresh_item", {
        itemId = itemId,
        inventoryName = inv.name,
        slotId = inv.items[itemId]
    })

    Timers:CreateTimer(function()
      if not IsValidEntity(item) then
        inv:RemoveItem(nil, true, {
          itemName = itemName,
          itemId = itemId,
          slotId = slotId
        })
      else
        inv:RefreshItem(itemId)
      end
    end)

    CustomGameEventManager:Send_ServerToPlayer(PlayerResource:GetPlayer(owner:GetPlayerOwnerID()), "start_item_cooldown", {itemId = itemId})
  end

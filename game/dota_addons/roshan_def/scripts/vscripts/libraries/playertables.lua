...
DEBUG_HIDE_SECTIONS["PlayerTables"] = true
...
--[[
  ...
  Library Usage
  -Non-Core --This block of commands has been added by JoBoDo and would have to be reimplemented when upgrading to future versions of PlayerTables
    -Lua
      -bool PlayerTables:HasTable(tableName)
...
function PlayerTables:HasTable(tableName)
...
function PlayerTables:CreateOrSubscribe(tableName, tableContents, pids)
  if PlayerTables:TableExists(tableName) then
    -- Subscribe
    PlayerTables:SetPlayerSubscriptions(tableName, pids)
  else
    -- Create
    PlayerTables:CreateTable(tableName, tableContents, pids)
  end
end
...
function PlayerTables:SetTableValue(tableName, key, value, bOverrideEqualityCheck)
  if value == nil then
  ...
  if not self:equals(table[key], value) then
    Debug("PlayerTables", "Update: ", tableName)
    table[key] = value
    for pid,v in pairs(pids) do
      local player = PlayerResource:GetPlayer(pid)
      if player then
    ...
    Debug("PlayerTables", "Tables in ", tableName, " are of equal values")
  end
end
...
function PlayerTables:SetTableValues(tableName, changes)
  ...
  for k,v in pairs(changes) do
    if self:equals(table[k], v) then
      Debug("PlayerTables", k, " was equal to old value ")
      --[[
        print("New")
        PrintTable(v)
        print("Old")
  ...
  Debug("PlayerTables", "1", tableName)
  ...
  if notempty then
    ...
    for pid,v in pairs(pids) do
      local player = PlayerResource:GetPlayer(pid)
      if player then
        Debug("PlayerTables", "Sending to client", tableName)
        CustomGameEventManager:Send_ServerToPlayer(player, "pt_uk", {name=tableName, changes=changes} )
      end
    end
  else
    Debug("PlayerTables", "Motherfucker was empty", tableName)
    PrintTable(changes)
  end
end

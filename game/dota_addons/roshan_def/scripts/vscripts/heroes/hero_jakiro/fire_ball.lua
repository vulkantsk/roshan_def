	...
	if not target then
		...
		for _, enemy in pairs(enemies_in_radius) do
			...
			if enemy:HasModifier("modifier_jakiro_dual_breath_new_fire") then --! Горение с 1 скилла
				...
				enemy:FindModifierByName("modifier_jakiro_dual_breath_new_ice"):SetDuration(enemy:FindModifierByName("modifier_jakiro_dual_breath_new_ice"):GetDuration() + self:GetSpecialValueFor("additional_duration"), true)

...
function Lift(keys)
	...
	target.lift_particle = ParticleManager:CreateParticle(keys.particle, PATTACH_WORLDORIGIN, caster)
	ParticleManager:SetParticleControl(target.lift_particle, 0, Vector(target:GetAbsOrigin().x,target:GetAbsOrigin().y,target:GetAbsOrigin().z))
	ParticleManager:SetParticleControl(target.lift_particle, 1, Vector(target:GetAbsOrigin().x,target:GetAbsOrigin().y,target:GetAbsOrigin().z))
	ParticleManager:SetParticleControl(target.lift_particle, 2, Vector(target:GetAbsOrigin().x,target:GetAbsOrigin().y,target:GetAbsOrigin().z))
...
function Fall(keys)
	...
	if target.point ~= nil then
		vector_distance = target:GetAbsOrigin()+ target:GetForwardVector() * 1000
	-- If there is no chosen drop position, it will be the same position the target was lifted from
	else
		target.point = target.target_origin  + target:GetForwardVector() * 1000
...
function Impact(keys)
	...
	if target.drop_particle ~= nil then
		ParticleManager:DestroyParticle(target.drop_particle, true)
	end

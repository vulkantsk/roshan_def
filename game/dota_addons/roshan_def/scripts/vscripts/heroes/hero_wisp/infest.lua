LinkLuaModifier( "modifier_wisp_powerup_dmg_buff", "heroes/hero_wisp/powerup.lua", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_wisp_powerup_armor_buff", "heroes/hero_wisp/powerup.lua", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_wisp_powerup_hp_buff", "heroes/hero_wisp/powerup.lua", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_wisp_powerup_as_buff", "heroes/hero_wisp/powerup.lua", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_wisp_powerup_ms_buff", "heroes/hero_wisp/powerup.lua", LUA_MODIFIER_MOTION_NONE )

LinkLuaModifier( "modifier_wisp_infest_passive", "heroes/hero_wisp/infest", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_wisp_infest_hidden", "heroes/hero_wisp/infest", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_wisp_infest_host", "heroes/hero_wisp/infest", LUA_MODIFIER_MOTION_NONE )

wisp_consume = class({})

function wisp_consume:OnSpellStart()
...
function wisp_infest:OnAbilityPhaseStart()
    local caster = self:GetCaster()
...
function wisp_infest:OnSpellStart( keys )
    local target = self:GetCursorTarget()
    local caster = self:GetCaster()
    local ability = self
	...
	caster:AddNewModifier(caster, ability, "modifier_wisp_infest_hidden", nil)
    caster.host = unit
	...
	unit:AddNewModifier(caster, ability, "modifier_wisp_infest_host", nil)
	...
	local effect = "particles/roshdef_io/wisp_mid.vpcf"
...
function wisp_infest:GetIntrinsicModifierName()
	return "modifier_wisp_infest_passive"
end

modifier_wisp_infest_passive = class({
...
function modifier_wisp_infest_passive:OnIntervalThink()
	local caster = self:GetCaster()
	local player = caster:GetPlayerID()
...
modifier_wisp_infest_hidden = class({
	IsHidden 				= function(self) return true end,
	IsPurgable 				= function(self) return false end,
	IsDebuff 				= function(self) return false end,
	IsBuff                  = function(self) return true end,
...
modifier_wisp_infest_host = class({
	IsHidden 				= function(self) return true end,
	IsPurgable 				= function(self) return false end,
	IsDebuff 				= function(self) return false end,
	IsBuff                  = function(self) return true end,
...
function modifier_wisp_infest_host:GetEffectName()
	return "particles/roshdef_io/roshdefio.vpcf"
end
function modifier_wisp_infest_host:GetEffectAttachType()
	return PATTACH_OVERHEAD_FOLLOW
end
function modifier_wisp_infest_host:OnDeath(data)
    local caster = self:GetCaster()
    ...
    if parent == unit  then 
		...
		caster:RemoveNoDraw()
		caster:RemoveModifierByName("modifier_wisp_infest_hidden")
		parent:RemoveModifierByName("modifier_wisp_infest_host")
...
function modifier_wisp_infest_host:OnCreated()
	local ability = self:GetAbility()
	local caster = self:GetCaster()
...
function modifier_wisp_infest_host:OnIntervalThink()
	local ability = self:GetAbility()
	local caster = self:GetCaster()
	local target = self:GetParent()
	...
	if target.init == nil then
		target.init = true
		self:GetParent():AddNewModifier(caster, nil, "modifier_phased", {duration = 0.1})
	end

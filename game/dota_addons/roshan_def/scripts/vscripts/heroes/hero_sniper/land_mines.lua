...
function LandMinesPlant( keys )
	...
	local land_mine = CreateUnitByName("npc_dota_techies_land_mine_creature", target_point, false, nil, nil, caster:GetTeamNumber())
...
function LandMineExplode( keys )
	...
	local agility_dmg = caster:GetAgility()*ability:GetSpecialValueFor("agi_dmg")

			local damageTable = {
        			victim = target,
        			attacker = caster,
        			damage = base_dmg+agility_dmg,

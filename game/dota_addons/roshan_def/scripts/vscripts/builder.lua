...
--[[
    ...
    event:OnConstructionStarted(function(unit)
        ...
        if building_name == "npc_dota_greevil_lord_chest" then
            if IsValidEntity(hero.bank) and hero.bank:IsAlive() then
        ...
        if building_name == "npc_dota_greevil_lord_tent_upgrader" then
            if IsValidEntity(hero.tent) and hero.tent:IsAlive() then
                hero.tent:ForceKill(false)
            end
            
            hero.tent = unit
        end

        if building_name == "npc_dota_greevil_lord_tower_upgrader" then
            if IsValidEntity(hero.statue) and hero.statue:IsAlive() then
                hero.statue:ForceKill(false)
            end
    ...
    event:OnConstructionCompleted(function(unit)
        ...
        local building_destrucion = unit:AddAbility("greevil_building_destruction")
        building_destrucion:SetLevel(1)
...
function UpgradeBuilding( event )
    ...
    newBuilding:AddAbility("greevil_building_destruction"):SetLevel(1)

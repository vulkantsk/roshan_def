...
LinkLuaModifier("modifier_imba_helldrain", "items/item_hellblade", LUA_MODIFIER_MOTION_NONE)
...
function item_imba_hellblade:OnSpellStart()
	if IsServer() then
		...
		if target:GetUnitName() ~= "npc_dota_sans" then
...
function modifier_imba_helldrain:OnIntervalThink()
	if IsServer() then
		...
		for _,enemy in pairs(enemies) do
			if enemy:GetUnitName() ~= "npc_dota_sans" or enemy:HasModifier("modifier_imba_helldrain_damage")  then

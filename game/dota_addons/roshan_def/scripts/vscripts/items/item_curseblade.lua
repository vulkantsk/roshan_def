...
function item_imba_curseblade:OnSpellStart()
	if IsServer() then
		...
		if target:GetUnitName() == "npc_dota_sans" and target:IsMagicImmune() then
			return nil
		end
...
function modifier_imba_souldrain:OnIntervalThink()
	if IsServer() then
		...
		for _,enemy in pairs(enemies) do
			if enemy:GetUnitName() ~= "npc_dota_sans" and enemy:HasModifier("modifier_imba_souldrain_damage") then

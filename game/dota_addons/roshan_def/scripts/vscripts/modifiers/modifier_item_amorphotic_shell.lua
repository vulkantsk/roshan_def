	...
	if IsServer() then
		if params.unit == self:GetParent() then
			if params.damage > 25 and RollPercentage( self.amoeba_chance ) then
				...
				local hAmoeba = CreateUnitByName( "npc_dota_water_elemental_1", vSpawnPos, true, self:GetParent(), self:GetParent(), self:GetParent():GetTeamNumber() )
				if hAmoeba ~= nil then
				...
				hAmoeba:SetControllableByPlayer(self:GetCaster():GetPlayerID(), false)
				hAmoeba:SetOwner(self:GetCaster())
			end	
		end
	end
	return 0

...
function modifier_spider_egg_sack:OnIntervalThink()
	if IsServer() then
		if self.bTriggered == false then
			local enemies = FindUnitsInRadius( self:GetParent():GetTeamNumber(), self:GetParent():GetOrigin(), self:GetParent(), self.trigger_radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, 0, false )
			if #enemies > 0 then
...
--[[
		for i=0,RandomInt( self.spider_min, self.spider_max ) do
			local hUnit = CreateUnitByName( "npc_dota_creature_spider_small", self:GetParent():GetOrigin(), true, nil, nil, DOTA_TEAM_BADGUYS )

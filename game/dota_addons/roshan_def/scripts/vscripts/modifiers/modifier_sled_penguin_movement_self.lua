...
modifier_sled_penguin_movement_self = class({})

----------------------------------------------------------------------------------

function modifier_sled_penguin_movement_self:IsHidden()
	return true
end

----------------------------------------------------------------------------------

function modifier_sled_penguin_movement_self:IsPurgable()
	return false
end
...
function modifier_sled_penguin_movement_self:GetModifierDisableTurning( params )
	return 1
end

-----------------------------------------------------------------------

function modifier_sled_penguin_movement_self:GetOverrideAnimation( params )
...
function modifier_sled_penguin_movement_self:OnCreated( kv )
	if IsServer() then
...
function modifier_sled_penguin_movement_self:OnIntervalThink()
	if IsServer() then
		if self:GetParent():HasModifier( "modifier_sled_penguin_movement_self" ) and self:GetParent():HasModifier( "modifier_sled_penguin_crash" ) == false then
			if self:GetParent().speed <= 750 then
				print(self:GetParent().speed)
			end

			if self:GetParent():GetUnitName() == "npc_dota_sled_penguin" or self:GetParent():GetUnitName() == "npc_dota_event_penguin" then
				...
				local hEnemies = FindUnitsInRadius( self:GetParent():GetTeamNumber(), self:GetParent():GetOrigin(), self:GetParent(), self.pinata_burst_radius, DOTA_UNIT_TARGET_TEAM_ENEMY, self:GetAbility():GetAbilityTargetType(), nTargetFlags, 0, false )
				for _, hEnemy in pairs( hEnemies ) do
					if hEnemy ~= nil and hEnemy:IsNull() == false and ( hEnemy:GetUnitName() == "npc_dota_coin_pinata" or hEnemy:GetUnitName() == "npc_dota_large_coin_pinata" ) then
						hEnemy:Kill( nil, self:GetParent() )
					end
				end
			end
		end
...
function modifier_sled_penguin_movement_self:OnDestroy()
	if IsServer() then
...
function modifier_sled_penguin_movement_self:DeclareFunctions()
	local funcs = 
	{
...
function modifier_sled_penguin_movement_self:GetModifierTurnRate_Override()
	return 0.5
end
--------------------------------------------------------------------------------

function modifier_sled_penguin_movement_self:CheckState()
	local state =
	{
...
function modifier_sled_penguin_movement_self:UpdateHorizontalMotion( me, dt )
	if IsServer() then
...
--			local flTurnRate = 125
			...
			self:GetParent().speed = self.nCurSpeed
			...
			if self:GetCaster():IsAlive() == false or self:GetCaster():FindModifierByName( "modifier_sled_penguin_movement_self" ) == nil then
				self:Destroy()
				return
			end
...
function modifier_sled_penguin_movement_self:CrashAndRecover()
	...
	self:GetParent():AddNewModifier( self:GetParent(), self:GetAbility(), "modifier_sled_penguin_movement_self", kv )
end

--------------------------------------------------------------------------------

--[[
function modifier_sled_penguin_movement_self:Crash()
...
function modifier_sled_penguin_movement_self:OnOrder( params )
	if IsServer() then
		if not self:GetCaster() then
			return 0
		end
		...
		if nOrderType == DOTA_UNIT_ORDER_MOVE_TO_POSITION or nOrderType == DOTA_UNIT_ORDER_ATTACK_MOVE then
			...
			if hOrderedUnit == self:GetParent()  then
				...
				local hBuff = self:GetCaster():FindModifierByName( "modifier_sled_penguin_movement_self" )
				if hBuff ~= nil then
...
function modifier_sled_penguin_movement_self:IsParentPenguin()

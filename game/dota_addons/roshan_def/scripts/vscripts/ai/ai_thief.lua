...
--	thisEntity:AddNewModifier( thisEntity, nil, "modifier_fow_vision", nil )
	...
	NoTargetAbility = thisEntity:FindAbilityByName( "ability_thief_1" )
	PointAbility = thisEntity:FindAbilityByName( "ability_thief_2" )
	TargetAbility = thisEntity:FindAbilityByName( "ability_thief_3" )
...
function ThisEntityThink()
	...
	if #enemies > 0 	then
		...
		if health_pct < 99 and NoTargetAbility ~= nil and NoTargetAbility:IsFullyCastable()then
			local heroes_in_radius = false
			for _,enemy in pairs(enemies) do
				if enemy:IsRealHero() then
					heroes_in_radius = true
					break
				end
			end
			if heroes_in_radius then  

        ...
        for i = 1, #allCreatures, 1 do
            ...
            if team ~= main_team then
                if GameRules.DIFFICULTY == 1 then
                    Spawn:UpgradeUnitStats(creature, 1.5)
                elseif GameRules.DIFFICULTY == 2 then
                    Spawn:UpgradeUnitStats(creature, 2)
                elseif GameRules.DIFFICULTY == 3 then
                    Spawn:UpgradeUnitStats(creature, 3)
                end
            end
        end
    end
...
--			print("key = "..key)
    ...
    if GameRules.ChaosMode == 1 then
        Spawn:ChaosTrigger(killedUnit)
    elseif GameRules.ZombieMode == 1 then
        Spawn:ZombieTrigger(killedUnit)
    end

    if name == "npc_dota_gold_mine" then
        GiveGoldPlayers(1000)
    end
    ...
    if name == "npc_dota_dbase2" then
        ...
        Timers:CreateTimer(1, function()
            Sounds:CreateGlobalSound("roshan_def.jojo")
        end)
        ...
        Timers:CreateTimer(45, function()
            ...
            if GameRules.ZombieMode == 1 then
                name = "npc_dota_zombie_lord"
            elseif GameRules.ChaosMode == 1 then
                name = "npc_dota_chaos_lord"
            elseif GameRules.DemonMode == 1 then
                name = "npc_dota_demon_lord"
                Sounds:CreateGlobalSound("terrorblade_spawn")
            elseif GameRules.LichMode == 1 then
                name = "npc_dota_lich_lord"
            elseif GameRules.NecroMode == 1 then
                name = "npc_dota_necro_lord"
            elseif GameRules.NecroMode == 0 then
                name = "npc_dota_necro_lord"
    ...
    elseif name == "npc_dota_terrboss" or name == "npc_dota_zombie_lord" or name == "npc_dota_chaos_lord" or name == "npc_dota_demon_lord" or name == "npc_dota_lich_lord" or name == "npc_dota_necro_lord" then
        ...
        Timers:CreateTimer(1, function()
            Sounds:CreateGlobalSound(sounds[RandomInt(1, #sounds)])
        end)
        GameRules.EndGame = 1
...
function Spawn:PrintEndgameMessage1()
    ...
    Timers:CreateTimer(5, function()
        GameRules:SendCustomMessage("<font color='#DBA901'><br>Game will end in 85 seconds</font>", 0, 0)
    end)
    ...
    --Send stats
    Timers:CreateTimer(87, function()
        GameRules:SendCustomMessage("<font color='#DBA901'>3</font>", 0, 0)
    end)
    Timers:CreateTimer(88, function()
        GameRules:SendCustomMessage("<font color='#DBA901'>2</font>", 0, 0)
    end)
    Timers:CreateTimer(89, function()
        GameRules:SendCustomMessage("<font color='#DBA901'>1...</font>", 0, 0)
    end)
    Timers:CreateTimer(90, function()
        request:GameEnd(DOTA_TEAM_GOODGUYS)
    end)
end
...
function Spawn:PrintEndgameMessage2()
    ...
    Timers:CreateTimer(5, function()
        GameRules:SendCustomMessage("<font color='#DBA901'><br>Game will end in 15 seconds</font>", 0, 0)
    end)
    Timers:CreateTimer(10, function()
        GameRules:SendCustomMessage("#Game_notification_end_game_message", 0, 0)
    end)
    ...
    Timers:CreateTimer(15, function()
        request:GameEnd(DOTA_TEAM_BADGUYS)
    end)
end
...
function Spawn:PrintFalseEndgameMessage()
    ...
    Timers:CreateTimer(5, function()
        GameRules:SendCustomMessage("<font color='#DBA901'><br>Conragulations !Game will end in 40 seconds</font>", 0, 0)
    end)
    Timers:CreateTimer(10, function()
        GameRules:SendCustomMessage("#Game_notification_end_game_message", 0, 0)
    end)
    ...
    Timers:CreateTimer(45, function()
        GameRules:SendCustomMessage("<font color='#DBA901'>Yes. I lie :) </font>", 0, 0)
    end)
...
function Spawn:OnNPCSpawned(keys)
    ...
    if (team ~= main_team and GameRules.DIFFICULTY == 1 and not npc:IsRealHero()) then
        Spawn:UpgradeUnitStats(npc, 1.5)
    elseif (team ~= main_team and GameRules.DIFFICULTY == 2 and not npc:IsRealHero()) then
        Spawn:UpgradeUnitStats(npc, 2)
    elseif (team ~= main_team and GameRules.DIFFICULTY == 3 and not npc:IsRealHero()) then
        Spawn:UpgradeUnitStats(npc, 3)
    end

    if (team == DOTA_TEAM_BADGUYS and GameRules.DemonMode == 1) then
        npc:AddNewModifier(npc, nil, "modifier_demon_lord_buff", {})
    elseif (team == DOTA_TEAM_GOODGUYS and GameRules.LichMode == 1) then
        npc:AddNewModifier(npc, nil, "modifier_lich_lord_debuff", {})
    elseif (team == DOTA_TEAM_GOODGUYS and GameRules.NecroMode == 1) then
        npc:AddNewModifier(npc, nil, "modifier_necro_lord_debuff", {})
    end
    ...
    if npc:IsRealHero() and npc.FirstSpawned == nil then
        ...
        then
            local ability = npc:FindAbilityByName("inherit_centaur_buff")
            ability:SetLevel(1)
            fractions["centaur"]["centaur"] = npc
            npc.race_buff = ability
        elseif name == "npc_dota_hero_leshrac"
        then
            local ability = npc:FindAbilityByName("inherit_leshrac_buff")
            ability:SetLevel(1)
            fractions["centaur"]["leshrac"] = npc
            npc.race_buff = ability
        elseif name == "npc_dota_hero_riki"
        then
            local ability = npc:FindAbilityByName("inherit_riki_buff")
            ability:SetLevel(1)
            fractions["satyr"]["riki"] = npc
            npc.race_buff = ability
        elseif name == "npc_dota_hero_lycan"
        then
            local ability = npc:FindAbilityByName("inherit_lycan_buff")
            ability:SetLevel(1)
            fractions["wolf"]["lycan"] = npc
            npc.race_buff = ability
        elseif name == "npc_dota_hero_ursa"
        then
            local ability = npc:FindAbilityByName("inherit_ursa_buff")
            ability:SetLevel(1)
            fractions["ursa"]["ursa"] = npc
            npc.race_buff = ability
        elseif name == "npc_dota_hero_lone_druid"
        then
            local ability = npc:FindAbilityByName("inherit_druid_buff")
            ability:SetLevel(1)
            fractions["ursa"]["druid"] = npc
            npc.race_buff = ability
        elseif name == "npc_dota_hero_jakiro"
        then
            local ability = npc:FindAbilityByName("inherit_jakiro_buff")
            ability:SetLevel(1)
            fractions["dragon"]["jakiro"] = npc
            npc.race_buff = ability
        elseif name == "npc_dota_hero_winter_wyvern"
        then
            local ability = npc:FindAbilityByName("inherit_wyvern_buff")
            ability:SetLevel(1)
            fractions["dragon"]["wyvern"] = npc
            npc.race_buff = ability
        elseif name == "npc_dota_hero_ogre_magi"
        then
            local ability = npc:FindAbilityByName("inherit_ogre_buff")
            ability:SetLevel(1)
            fractions["ogre"]["ogre_magi"] = npc
            npc.race_buff = ability
        elseif name == "npc_dota_hero_tiny"
        then
            local ability = npc:FindAbilityByName("inherit_tiny_buff")
            ability:SetLevel(1)
            fractions["golem"]["tiny"] = npc
            npc.race_buff = ability
        elseif name == "npc_dota_hero_troll_warlord"
        then
            local ability = npc:FindAbilityByName("inherit_troll_buff")
            ability:SetLevel(1)
            fractions["troll"]["troll"] = npc
            npc.race_buff = ability
        elseif name == "npc_dota_hero_huskar"
        then
            local ability = npc:FindAbilityByName("inherit_huskar_buff")
            ability:SetLevel(1)
            fractions["troll"]["huskar"] = npc
            npc.race_buff = ability
        elseif name == "npc_dota_hero_meepo"
        then
            local ability = npc:FindAbilityByName("inherit_meepo_buff")
            ability:SetLevel(1)
            fractions["kobold"]["meepo"] = npc
            npc.race_buff = ability
        elseif name == "npc_dota_hero_pangolier"
        then
            local ability = npc:FindAbilityByName("pangolier_greed")
            ability:SetLevel(1)
        elseif name == "npc_dota_hero_beastmaster"
        then
            local ability = npc:FindAbilityByName("inherit_beastmaster_buff")
            ability:SetLevel(1)
            fractions["beastmaster"] = npc
            npc.race_buff = ability
        elseif name == "npc_dota_hero_chen"
        then
            local ability = npc:FindAbilityByName("inherit_chen_buff")
            ability:SetLevel(1)
            fractions["chen"] = npc
            npc.race_buff = ability
        elseif name == "npc_dota_hero_dragon_knight"
        then
            local ability = npc:FindAbilityByName("inherit_antares_buff")
            ability:SetLevel(1)
            fractions["antares"] = npc
            npc.race_buff = ability
        elseif name == "npc_dota_hero_disruptor"
        then
            local ability = npc:FindAbilityByName("inherit_disruptor_buff")
            ability:SetLevel(1)
        ...
        then
            local ability = npc:FindAbilityByName("wisp_powerup_dmg")
            ability:SetLevel(1)
        ...
        then
            local ability = npc:FindAbilityByName("jacob_powerup_dmg")
            ability:SetLevel(1)
        ...
        then
            local ability = npc:FindAbilityByName("mark_strength")
            ability:SetLevel(1)
...
function Spawn:SecretBoxSpawn()
    -- Функция начнет выполняться, когда начнется матч( на часах будет 00:00 ).
    print("that work !!!")
...
function Spawn:SecretTreeSpawn()
    -- Функция начнет выполняться, когда начнется матч( на часах будет 00:00 ).
...
function Spawn:DireTideTreeSpawner()
    -- Функция начнет выполняться, когда начнется матч( на часах будет 00:00 ).
    for i = 3, 11 do
        for k = 1, 5 do
...
function Spawn:DireSpawner1()
    ...
    Timers:CreateTimer(20, function()
        ...
        for i = 1, 4 do
            -- Произведет нижние действия столько раз, сколько указано в ROUND_UNITS. То есть в нашем случае создаст 2 юнита.
        ...
        if self.DireSpawner ~= 1 then
            return return_time
        end-- Возвращаем таймеру время, через которое он должен снова сработать. Когда пройдет последний раунд таймер получит значение 'nil' и выключится.
    end)
end
function Spawn:DireSpawner2()
    -- Функция начнет выполняться, когда начнется матч( на часах будет 00:00 ).
    ...
    Timers:CreateTimer(10, function()
        ...
        if GameRules.NecroMode == 1 then
            unit = CreateUnitByName("npc_dota_necro_spawn", point1 + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_BADGUYS)
        else
            unit = CreateUnitByName("npc_dota_direr1", point1 + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_BADGUYS)
        end
        unit:SetInitialGoalEntity(waypoint) -- Посылаем мобов на наш way1, координаты которого мы записали в переменную 'waypoint'


        if GameRules.LichMode == 1 then
            unit = CreateUnitByName("npc_dota_lich_spawn", point1 + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_BADGUYS)
        else
            unit = CreateUnitByName("npc_dota_direr2", point1 + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_BADGUYS)
        end
        unit:SetInitialGoalEntity(waypoint) -- Посылаем мобов на наш way1, координаты которого мы записали в переменную 'waypoint'
        ...
        if GameRules.RoshanUpgrade ~= 1 and GameRules.MegaMode ~= 1 then
            ...
            for i = 1, 4 do
                ...
                if RandomInt(1, 100) > 20 then
                    unit = CreateUnitByName("npc_dota_kobold2", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                else
                    unit = CreateUnitByName("npc_dota_kobold3", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                end
            ...
            if (self.golem == 2) then
                local unit
                if RandomInt(1, 100) > 20 then
                    unit = CreateUnitByName("npc_dota_golem3", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                else
                    unit = CreateUnitByName("npc_dota_golem4", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                end
                AttackMove(unit, dire_point)
                --				unit:SetInitialGoalEntity( waypoint ) -- Посылаем мобов на наш way1, координаты которого мы записали в переменную 'waypoint'
            elseif (self.golem == 1) then
                local unit
                if RandomInt(1, 100) > 20 then
                    unit = CreateUnitByName("npc_dota_golem1", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                else
                    unit = CreateUnitByName("npc_dota_golem2", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                end
                AttackMove(unit, dire_point)
                --				unit:SetInitialGoalEntity( waypoint ) -- Посылаем мобов на наш way1, координаты которого мы записали в переменную 'waypoint'
            ...
            if self.ursa == 1 then
                local unit
                if RandomInt(1, 100) > 20 then
                    unit = CreateUnitByName("npc_dota_ursa1", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                else
                    unit = CreateUnitByName("npc_dota_ursa2", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                end
                AttackMove(unit, dire_point)
                --				unit:SetInitialGoalEntity( waypoint ) -- Посылаем мобов на наш way1, координаты которого мы записали в переменную 'waypoint'
            end
            if self.ogre == 1 then
                ...
                if RandomInt(1, 100) > 20 then
                    unit = CreateUnitByName("npc_dota_ogre1", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                else
                    unit = CreateUnitByName("npc_dota_ogre2", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                end
                AttackMove(unit, dire_point)
                --				unit:SetInitialGoalEntity( waypoint ) -- Посылаем мобов на наш way1, координаты которого мы записали в переменную 'waypoint'
            end
            if self.DireSpawner ~= 1 then
                local unit
                if RandomInt(1, 100) > 20 then
                    unit = CreateUnitByName("npc_dota_troll1", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                else
                    unit = CreateUnitByName("npc_dota_troll2", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                end
            ...
            elseif self.DireSpawner == 1 then
                local unit
                if RandomInt(1, 100) > 20 then
                    unit = CreateUnitByName("npc_dota_troll3", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                else
                    unit = CreateUnitByName("npc_dota_troll4", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                end
                AttackMove(unit, dire_point)
                --				 unit:SetInitialGoalEntity( waypoint ) -- Посылаем мобов на наш way1, координаты которого мы записали в переменную 'waypoint'
            end
            ...
            if self.wolf == 1 then
                local unit
                if RandomInt(1, 100) > 20 then
                    unit = CreateUnitByName("npc_dota_wolf1", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                else
                    unit = CreateUnitByName("npc_dota_wolf2", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                end
                AttackMove(unit, dire_point)
                --				unit:SetInitialGoalEntity( waypoint ) -- Посылаем мобов на наш way1, координаты которого мы записали в переменную 'waypoint'
            end
            if self.satyr == 1 then
                local unit
                if RandomInt(1, 100) > 20 then
                    unit = CreateUnitByName("npc_dota_satyr2", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                else
                    unit = CreateUnitByName("npc_dota_satyr3", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                end
                AttackMove(unit, dire_point)
                --			 unit:SetInitialGoalEntity( waypoint ) -- Посылаем мобов на наш way1, координаты которого мы записали в переменную 'waypoint'
            end
            if self.dragon == 1 then
                local unit
                if RandomInt(1, 100) > 20 then
                    unit = CreateUnitByName("npc_dota_dragon1", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                else
                    unit = CreateUnitByName("npc_dota_dragon2", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                end
                AttackMove(unit, dire_point)
                --				unit:SetInitialGoalEntity( waypoint ) -- Посылаем мобов на наш way1, координаты которого мы записали в переменную 'waypoint'
            end
            if self.lizard == 1 then
                local unit
                if RandomInt(1, 100) > 20 then
                    unit = CreateUnitByName("npc_dota_lizard1", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                else
                    unit = CreateUnitByName("npc_dota_lizard2", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                end
                AttackMove(unit, dire_point)
                --				unit:SetInitialGoalEntity( waypoint ) -- Посылаем мобов на наш way1, координаты которого мы записали в переменную 'waypoint'
            end
            if self.centaur == 1 then
                local unit
                if RandomInt(1, 100) > 20 then
                    unit = CreateUnitByName("npc_dota_centaur1", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                else
                    unit = CreateUnitByName("npc_dota_centaur2", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                end
                AttackMove(unit, dire_point)
                --			unit:SetInitialGoalEntity( waypoint ) -- Посылаем мобов на наш way1, координаты которого мы записали в переменную 'waypoint'
            end
...
function Spawn:NeutralSpawnerEvent()
    -- Функция начнет выполняться, когда начнется матч( на часах будет 00:00 ).
    local point = Entities:FindByName(nil, "nmpoint"):GetAbsOrigin()
    local point1 = Entities:FindByName(nil, "nrpoint"):GetAbsOrigin()
    local waypoint = Entities:FindByName(nil, "n_waypoint1")
...
function Spawn:MegaNeutralSpawner()
    -- Функция начнет выполняться, когда начнется матч( на часах будет 00:00 ).
    local point = Entities:FindByName(nil, "nmpoint"):GetAbsOrigin()
    local point1 = Entities:FindByName(nil, "nrpoint"):GetAbsOrigin()
    local waypoint = Entities:FindByName(nil, "n_waypoint1")
...
function Spawn:NeutralSpawner1()
    -- Функция начнет выполняться, когда начнется матч( на часах будет 00:00 ).
    local point = Entities:FindByName(nil, "nmpoint"):GetAbsOrigin()
    local point1 = Entities:FindByName(nil, "nrpoint"):GetAbsOrigin()
    local waypoint = Entities:FindByName(nil, "n_waypoint1")
        ...
        if GameRules.MegaMode ~= 1 then
            ...
            for i = 1, 4 do
                ...
                if RandomInt(1, 100) < 5 then
                    unit = CreateUnitByName("npc_dota_roshan3", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                else
                    unit = CreateUnitByName("npc_dota_roshan_kobold", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                end
                unit:SetInitialGoalEntity(waypoint) -- Посылаем мобов на наш way1, координаты которого мы записали в переменную 'waypoint'
            end
            if (self.golem == 1) then
                ...
                if RandomInt(1, 100) < 5 then
                    ...
                    unit = CreateUnitByName("npc_dota_roshan_golem", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                end
                unit:SetInitialGoalEntity(waypoint) -- Посылаем мобов на наш way1, координаты которого мы записали в переменную 'waypoint'
            ...
            if self.ursa == 1 then
                ...
                if RandomInt(1, 100) < 5 then
                    ...
                    unit = CreateUnitByName("npc_dota_roshan_ursa", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                end
                unit:SetInitialGoalEntity(waypoint) -- Посылаем мобов на наш way1, координаты которого мы записали в переменную 'waypoint'
            end
            if self.ogre == 1 then
                ...
                if RandomInt(1, 100) < 5 then
                    ...
                    unit = CreateUnitByName("npc_dota_roshan_ogre", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                end
                unit:SetInitialGoalEntity(waypoint) -- Посылаем мобов на наш way1, координаты которого мы записали в переменную 'waypoint'
            end
            ...
            if RandomInt(1, 100) < 5 then
                ...
                unit = CreateUnitByName("npc_dota_roshan_troll", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
            end
            ...
            if self.wolf == 1 then
                ...
                if RandomInt(1, 100) < 5 then
                    ...
                    unit = CreateUnitByName("npc_dota_roshan_wolf", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                end
                unit:SetInitialGoalEntity(waypoint) -- Посылаем мобов на наш way1, координаты которого мы записали в переменную 'waypoint'
            end
            if self.satyr == 1 then
                ...
                if RandomInt(1, 100) < 5 then
                    ...
                    unit = CreateUnitByName("npc_dota_roshan_satyr", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                end
                unit:SetInitialGoalEntity(waypoint) -- Посылаем мобов на наш way1, координаты которого мы записали в переменную 'waypoint'
            end
            if self.dragon == 1 then
                ...
                if RandomInt(1, 100) < 5 then
                    ...
                    unit = CreateUnitByName("npc_dota_roshan_dragon", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                end
                unit:SetInitialGoalEntity(waypoint) -- Посылаем мобов на наш way1, координаты которого мы записали в переменную 'waypoint'
            end
            if self.centaur == 1 then
                ...
                if RandomInt(1, 100) < 5 then
                    ...
                    unit = CreateUnitByName("npc_dota_roshan_centaur", point + RandomVector(RandomFloat(0, 200)), true, nil, nil, DOTA_TEAM_GOODGUYS)
                end
                unit:SetInitialGoalEntity(waypoint) -- Посылаем мобов на наш way1, координаты которого мы записали в переменную 'waypoint'
            end
        end
...
function Spawn:DireBossSpawner()
    -- Функция начнет выполняться, когда начнется матч( на часах будет 00:00 ).
    local point = Entities:FindByName(nil, "dmpoint2"):GetAbsOrigin()
    local point1 = Entities:FindByName(nil, "drpoint2"):GetAbsOrigin()

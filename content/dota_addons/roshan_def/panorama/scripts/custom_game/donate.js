...
function UploadDonatePanel(){ 
	let heroes_container = $('#HeroesList')
	let items_container  = $('#ItemsList')
	items_container.RemoveAndDeleteChildren()
	heroes_container.RemoveAndDeleteChildren()
	let config = CustomNetTables.GetTableValue('request', 'item_config')
	let items_obj = CustomNetTables.GetTableValue('request', 'Player_' + Game.GetLocalPlayerInfo().player_id )
	items_obj = items_obj ? items_obj['Items'] : [];
	let items = []
	_.each(items_obj,value => {
		items.push(value)
	})    
  
	if (!config) return 
 
	_.each(config,(data,key) =>{
		let includes_ = items.includes(key - 1);
		if (data.all_hidden == 1 && !includes_) return

		let panel = $.CreatePanel('DOTAItemImage',data.item == 1 ? items_container : heroes_container,data.item_name)
		panel.itemname = data.item_name
		panel.enabled = includes_

		panel.SetPanelEvent('onactivate', () =>{
			GameEvents.SendCustomGameEventToServer( "donate_pick_item", {  
				item_id:  key,
			});
			panel.enabled = false
		})
...
( function () {
	...
	let IsDonateHidden = CustomNetTables.GetTableValue('donate', 'GLOBAL')
	IsDonateHidden = IsDonateHidden && IsDonateHidden.use_donate == 1
	if (IsDonateHidden) 
		$('#DonateToggleButton').style.visibility = 'collapse';
	UploadDonatePanel()
} )();